...
func httpHandler(response http.ResponseWriter, request *http.Request) {
	...
	defer func() {
		if status/100 > 2 {
			logger.Warn(map[string]interface{}{"protocol": "http", "event": "response", "remote": request.RemoteAddr, "file": file, "status": status})
		} else {
			elapsed := int(time.Now().Sub(sstart) / time.Millisecond)
			if elapsed == 0 {
				elapsed = 1
			}
			logger.Info(map[string]interface{}{"protocol": "http", "event": "response", "remote": request.RemoteAddr, "file": file, "mode": mode,
				"status": status, "size": tsize, "sent": sent, "duration": elapsed, "bandwidth": (sent * 8) / elapsed})
		}
	}()
			...
			if match := config.GetString("routes."+route+".match", ""); match != "" {
				...
				found:
					for _, path := range config.GetPaths("routes." + route + ".backends") {
						...
						switch mode {
						case "file":
							tsize, _ = fileBackend(target, 0, 1)
						case "http":
							...
							tsize, _ = httpBackend(target, 0, 1, timeout, headers)
						case "exec":
	...
	toffset := start
	for {
		bsize := int(math.Min(float64(end-toffset+1), 8<<20))
		if bsize <= 0 {
			break
		}
		switch mode {
		case "file":
			_, content = fileBackend(target, toffset, bsize)
		case "http":
			_, content = httpBackend(target, toffset, bsize, timeout, headers)
		case "exec":
